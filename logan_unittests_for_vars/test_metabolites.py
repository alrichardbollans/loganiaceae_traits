...
from logan_metabolite_vars import logan_accepted_metabolites_output_csv, \
    logan_unaccepted_metabolites_output_csv, logan_metabolites_output_csv
...
class MyTestCase(unittest.TestCase):
    ...
    def test_meta_hits(self):
        import numpy as np
        accepted_df = pd.read_csv(logan_accepted_metabolites_output_csv, index_col=0)
        unaccepted_df = pd.read_csv(logan_unaccepted_metabolites_output_csv, index_col=0)
        cat_temp_outputs = pd.concat([accepted_df, unaccepted_df])
        final_output_df = pd.read_csv(logan_metabolites_output_csv, index_col=0)

        # Check both_df has unqiue accepted names
        ...
        for i in tqdm(range(len(cat_temp_outputs.columns)), desc="testing", ascii=False, ncols=72):
            ...
            if c not in ['taxa'] + list(COL_NAMES.values()):
                ...
                should_be_one = should_be_one[
                    should_be_one['Accepted_Name'].isin(accepted_df['Accepted_Name'].values)]
